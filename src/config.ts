    ...
    try {
        ...
        if (yaml.logging !== undefined) {
            ...
            result.logging = {
                ...
                db_path: yaml.logging.db_path || './logs/vercel-dev-router.db',
            };
        }
        // ★ 工具处理配置
...
function applyEnvOverrides(cfg: AppConfig): void {
    ...
    if (process.env.LOG_FILE_ENABLED !== undefined) {
        if (!cfg.logging) cfg.logging = { file_enabled: false, dir: './logs', max_days: 7, persist_mode: 'summary', db_enabled: false, db_path: './logs/vercel-dev-router.db' };
